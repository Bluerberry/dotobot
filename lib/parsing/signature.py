...
class Signature:
	...
	def match(self, command: list[core.Token]) -> MatchResult:
		matched_parameters   = {}
		matched_flags        = []
		matched_variables    = {}
		...
		def internal(signature: core.Token):
			...
			# Matching parameters
			elif isinstance(signature, Parameter):

				# Check if atleast one parameter matches
				if not command or command[0] != signature:
					return not signature.required

				# Gather all matching parameters
				if signature.array:
					matched_parameters[signature.label] = []
					while command and command[0] == signature:
						matched_parameters[signature.label].append(command.pop(0).value)

				# Gather first matching parameter
				else:
					matched_parameters[signature.label] = command.pop(0).value

				return True

			# Matching flags
			elif isinstance(signature, Flag):
			...
			elif isinstance(signature, Variable):
